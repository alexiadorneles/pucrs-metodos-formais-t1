    ...
    ensures Conteudo == old(Conteudo) + [e]
  {
    ...
    if (size == a.Length) {
      ...
      while i < a.Length
        ...
        // invariant forall k : nat :: 0 <= k < i ==> (exists l:nat :: 0 <= l < a.Length && a[l] == novoArray[k])
        // invariant fresh(Repr - old(Repr))
        // invariant fresh(novoArray)
      {
        novoArray[i] := a[i];
        j:=(j+1) % a.Length;
        i := i + 1;
...
method Main()
{
  ...
  // assert fila.Conteudo == [1,2,3, 4, 5, 6];
